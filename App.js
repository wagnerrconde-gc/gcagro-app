...
const ICON_PATHS = {
  ...
  flask: <>
    <path d="M8 3.5H12"/>
    <path d="M8.7 3.5V8.5L4.3 15.3C3.9 16 4.4 17 5.3 17H14.7C15.6 17 16.1 16 15.7 15.3L11.3 8.5V3.5"/>
    <path d="M6.3 12.5H13.7"/>
  </>,
};
...
// Larguras fixas (px) por coluna da tabela de produtos da Programação — compartilhadas entre
// as tabelas de todas as categorias (Adubação, Sementes, TS...), que senão calculam a largura de
...
const CATEGORIAS_COMPRA_PADRAO = ["Sementes Verão","Sementes Inverno","Adubação Verão","Adubação Inverno","Químicos Verão","Químicos Inverno"];
const CATEGORIA_COMPRA_ICONS = {
  "Sementes Verão":"🌾","Sementes Inverno":"🌾","Adubação Verão":"🌱","Adubação Inverno":"🌱",
...
function calcQtdSementes(row) {
  ...
  const qtd = (pop * 20000 * area) / SEMENTES_POR_UNIDADE[unidade];
  // +3% de margem de segurança em cima da quantidade já em sc/bag, pra não faltar semente.
  return qtd * 1.03;
}
                      ...
                      <td key={field} className={printHide?"print-hide":undefined} style={{padding:"3px 5px"}}>
                        {type==="select" ? (
                          ...
                          (()=>{ const qtd=calcQtdSementes(row); return (
                            <div style={{padding:"3px 5px",textAlign:dataAlign||"center",fontWeight:qtd!=null?700:400,color:qtd!=null?cc.bg:"#bbb",minWidth:width||70}} title={qtd!=null?"Calculado: População × 20000 × Área ÷ sementes por unidade, +3% de margem em cima da quantidade em sc/bag":"Fórmula disponível apenas para Soja e Milho"}>
                              {qtd!=null?fmtN(qtd,1):"—"}
                            </div>
                          );})()
                        ) : type==="unit" ? (
...
function App() {
  ...
  // No celular dá pra ver a Programação de dois jeitos, e a escolha fica guardada: "cartoes"
  // (resumo por produto, sem rolar pro lado) ou "tabela" (as mesmas colunas do computador, com
  // rolagem lateral e a coluna Produto fixa) — útil pra conferir a lavoura inteira de relance.
  const [progMobileView, setProgMobileView] = useState(() => loadLS(KEY_PROG_MOBILE_VIEW, "cartoes"));
  useEffect(() => { saveLS(KEY_PROG_MOBILE_VIEW, progMobileView); }, [progMobileView]);
  ...
  // exatamente "Sementes") viram "Sementes Verão" por padrão. Idempotente — some sozinho depois.
  useEffect(() => {
    if (comprasRecords.some(r => r.categoria === "Sementes")) {
  ...
  // na Programação", só que aqui é só uma prévia (não grava nada). A pasta já vem separada por
  // temporada (igual Adubação/Químicos), então a temporada é lida do nome da categoria aberta.
  const mediasSementesPorCultura = useMemo(() => {
    if (!comprasCatSel || !comprasCatSel.startsWith("Sementes") || !comprasSafraSel) return [];
    const recs = comprasRecords.filter(r=>r.safra===comprasSafraSel && r.categoria===comprasCatSel);
    const isVerao = comprasCatSel.includes("Verão");
    const label = isVerao ? "Verão" : "Inverno";
    const dProg = isVerao ? dataVerao : dataInverno;
  ...
  const MAIN_TABS = [
    ...
    { id:"pecas",          label:"Estoque de Peças",      icon:"bolt", group:null },
    { id:"insumos",        label:"Estoque de Insumos",    icon:"flask", group:null },
    ...
    <div className={printAplicacaoId?"print-hide":undefined} style={{minHeight:"100vh",fontFamily:"system-ui,sans-serif"}}>
      ...
      {appView==="dashboard" && (()=>{
        ...
        const NAV_CARDS = [
          ...
          { id:"pecas",       label:"Estoque de Peças",      icon:"bolt",    color:"#5d4037", group:"Operação" },
          { id:"insumos",     label:"Estoque de Insumos",    icon:"flask",   color:"#334155", group:"Operação" },
      ...
      {(appView==="prog_verao"||appView==="prog_inv") && (
        <div style={{padding:"16px 12px"}}>
          ...
          {/* Como ver a Programação no celular: cartões (resumo, sem rolar pro lado) ou a tabela
              inteira com rolagem lateral. A escolha fica guardada no aparelho. */}
          {isMobile && (
            <div style={{display:"flex",alignItems:"center",justifyContent:"flex-end",gap:6,margin:"2px 0 10px"}}>
              <div style={{display:"flex",alignItems:"center",gap:6,background:"#fff",borderRadius:18,padding:"4px 6px 4px 12px",boxShadow:"0 1px 4px rgba(0,0,0,0.15)"}}>
                <span style={{fontSize:11,color:"#777"}}>Ver como</span>
                {[{id:"cartoes",txt:"🗂 Cartões"},{id:"tabela",txt:"📊 Tabela"}].map(op=>(
                  <button key={op.id} onClick={()=>setProgMobileView(op.id)}
                    style={{padding:"5px 12px",borderRadius:14,fontSize:11,fontWeight:progMobileView===op.id?700:400,cursor:"pointer",
                      background:progMobileView===op.id?colors.bg:"#f1f1f1",
                      color:progMobileView===op.id?"#fff":"#666",border:"none"}}>{op.txt}</button>
                ))}
              </div>
            </div>
          )}
      ...
      {appView==="insumos" && (()=>{
        ...
        const totaisPorCategoria = GRUPOS_ESTOQUE_INSUMOS.map(cat => {
          const recs = insumosEstoqueRecords.filter(r=>r.categoria===cat);
          return { categoria: cat, count: recs.length, valor: recs.reduce((s,r)=>s+(r.quantidade||0)*(r.custoMedio||0),0) };
        });
        const valorTotalEstoque = insumosEstoqueRecords.reduce((s,r)=>s+(r.quantidade||0)*(r.custoMedio||0),0);
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12,marginBottom:14}}>
              {totaisPorCategoria.map(t=>(
                <div key={t.categoria} onClick={()=>setInsumoEstoqueCatFiltro(t.categoria)}
                  ...
                  <div style={{fontSize:11,color:"#888",marginTop:2}}>{t.valor?fmt(t.valor):"—"}</div>
                </div>
              ))}
              <div style={{...cardSt,background:"#334155"}}>
                <div style={{fontSize:11,color:"#cbd5e1"}}>Valor total do estoque</div>
                <div style={{fontSize:18,fontWeight:800,color:"#fff"}}>{fmt(valorTotalEstoque)}</div>
              </div>
            </div>
              ...
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                ...
                <tbody>
                  ...
                  {comprasRecordsFiltrados.length===0 && !addingCompra && (
                    <tr><td colSpan={9 + ((comprasCatSel||"").startsWith("Sementes")?1:0) + (comprasRevendaSel?1:0)} style={{padding:"20px",textAlign:"center",color:"#bbb",fontSize:12}}>
      ...
      {showFecharCotModal && (()=>{
        ...
        function confirmar() {
          ...
          const categoria = isAdubFechar ? "Adubação "+temporadaLabel : isSemFechar ? "Sementes "+temporadaLabel : "Químicos "+temporadaLabel;
